...
static int detect_yolact(const cv::Mat& bgr, std::vector<Object>& objects, uint8_t n_threads)
{
...
static void draw_objects(const cv::Mat& bgr, const std::vector<Object>& objects)
{
    ...
    double fps = 1.0/(current_time-last_time).toSec();
    last_time = current_time;

    cv::putText(image, std::to_string(fps)+" Hz", cv::Point(20, 20), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255));
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg, int n_threads)
{
  try {
    ...
    detect_yolact(cv_ptr->image, objects, n_threads);
    if (display_output) draw_objects(cv_ptr->image, objects);
  }
...
{
  ros::init(argc, argv, "yolact_gpu_node");
  ...
  g_vkdev = ncnn::get_gpu_device(0); // gpu_device
  g_blob_vkallocator = new ncnn::VkBlobAllocator(g_vkdev);
  g_staging_vkallocator = new ncnn::VkStagingAllocator(g_vkdev);

  yolact.opt.use_vulkan_compute = true;
  yolact.set_vulkan_device(g_vkdev);
  yolact.load_param((path+("yolact.param")).c_str());
  yolact.load_model((path+("yolact.bin")).c_str());
