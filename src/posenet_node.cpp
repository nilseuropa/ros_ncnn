...
std::vector<KeyPoint> objects; /* o.O */
cv_bridge::CvImagePtr cv_ptr;
ros::Time last_time;
bool display_output;

void print_objects(const std::vector<KeyPoint>& objects){
    for (size_t i = 0; i < objects.size(); i++)
    {
      const KeyPoint& obj = objects[i];
      if (obj.prob > 0.2) ROS_INFO("%.5f at %.2f %.2f", obj.prob, obj.p.x, obj.p.y);
    }
}
...
{
  ros::init(argc, argv, "posenet_node"); /**/
  ...
  engine.neuralnet.load_param((path+("pose.param")).c_str()); /**/
  engine.neuralnet.load_model((path+("pose.bin")).c_str()); /**/
